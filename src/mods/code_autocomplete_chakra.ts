...
const chakra_autocomplete: Array<[string, string, string, string, number?]> = [
  ['={[', `(?:\\s|^)([^\\s]+={\\[)$`, '\n', 'null,null]}\n', 27],
  [
    '<Text ',
    `<Text $`,
...
export default async (event: any) => {
  ...
  for (const [
    ...
    chakra_add_before,
    chakra_add_after,
    chakra_go_back_length,
  ] of chakra_autocomplete) {
    ...
    if (textBeforeTrigger.endsWith(chakra_trigger)) {
      ...
      if (match) {
        ...
        await editor.edit((editBuilder) => {
          editBuilder.replace(
            new vscode.Range(replaceStartPosition, adjustedChangeEndPosition),
            chakra_add_before + matchedText + chakra_add_after
          );
        });
        /*
